...
use mi_plaits_dsp::engine::{
    waveshaping_engine::WaveshapingEngine, Engine, EngineParameters, TriggerState,
};
...
#[serde(default)]
struct WaveshapingOscillatorParams {
    /// frequency in v/oct
    freq: Signal,
    /// timbre parameter (0-1): wavefolder amount
    timbre: Signal,
    /// morph parameter (0-1): waveform asymmetry
    morph: Signal,
    /// harmonics parameter (0-1): waveshaper waveform
    harmonics: Signal,
...
#[module("mi.waveshape", "Waveshaping oscillator")]
#[args(freq)]
pub struct WaveshapingOscillator {
    outputs: WaveshapingOscillatorOutputs,
    engine: Option<WaveshapingEngine>,
    ...
    params: WaveshapingOscillatorParams,
}

impl WaveshapingOscillator {
    fn update(&mut self, sample_rate: f32) {
        if self.engine.is_none() || (self.sample_rate - sample_rate).abs() > 0.1 {
            let mut engine = WaveshapingEngine::new();
        ...
        self.outputs.sample = crate::dsp::utils::map_range(
            self.buffer_out[self.buffer_pos],
            -1.0,
            1.0,
            min,
            max,
...
message_handlers!(impl WaveshapingOscillator {});
