...
struct HihatOscillatorOutputs {
    #[output("output", "signal output", range = (-2.2, 2.2))]
    sample: f32,
...
#[module("mi.hihat", "Hi-hat synthesis")]
#[args(freq)]
pub struct HihatOscillator {
    outputs: HihatOscillatorOutputs,
    engine: Option<HihatEngine>,
    buffer_out: Vec<f32>,
    buffer_aux: Vec<f32>,
    buffer_pos: usize,
    last_sync: f32,
    ...
    morph: Clickless,
    harmonics: Clickless,
    params: HihatOscillatorParams,
}

impl HihatOscillator {
    fn update(&mut self, sample_rate: f32) {
        if self.engine.is_none() || (self.sample_rate - sample_rate).abs() > 0.1 {
            let mut engine = HihatEngine::new(BLOCK_SIZE);
...
message_handlers!(impl HihatOscillator {});
