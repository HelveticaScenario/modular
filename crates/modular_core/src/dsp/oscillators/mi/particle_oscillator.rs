...
use mi_plaits_dsp::engine::{
    Engine, EngineParameters, TriggerState, particle_engine::ParticleEngine,
};
...
#[module("mi.particle", "Particle noise synthesis")]
#[args(freq)]
pub struct ParticleOscillator {
    outputs: ParticleOscillatorOutputs,
    ...
    morph: Clickless,
    harmonics: Clickless,
    params: ParticleOscillatorParams,
}

impl ParticleOscillator {
    fn update(&mut self, sample_rate: f32) {
        if self.engine.is_none() || (self.sample_rate - sample_rate).abs() > 0.1 {
            let mut engine = ParticleEngine::new(BLOCK_SIZE);
...
message_handlers!(impl ParticleOscillator {});
