    ...
    while let Ok(cmd) = self.command_rx.pop() {
      match cmd {
        ...
        GraphCommand::SingleModuleUpdate {
          ...
          self
            .patch
            .add_message_listeners_for_module(&module_id);
...
mod tests {
  ...
  use modular_core::types::{Message, MessageTag, MidiNoteOn, MessageHandler};
  ...
  fn test_single_module_update_re_registers_message_listeners() {
    ...
    let mut processor = AudioProcessor::new(cmd_consumer, err_producer, garbage_producer, shared, 44100.0);
    ...
    cmd_producer
      ...
      .unwrap();
    cmd_producer
      .push(GraphCommand::DispatchMessage(Message::MidiNoteOn(MidiNoteOn {
        device: None,
        channel: 0,
        note: 60,
        velocity: 100,
  ...
  fn test_single_module_update_refreshes_patch_update_caches() {
    ...
    processor.patch.sampleables.insert(
      "src".into(),
      ConstantOutputModule::new("src", 3.5),
    );
    processor.patch.sampleables.insert(
      "dep".into(),
