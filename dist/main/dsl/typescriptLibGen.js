...
exports.buildLibSource = buildLibSource;
...
function renderReadonlyPropertyKey(name) {
    return isValidIdentifier(name) ? name : `[${JSON.stringify(name)}]`;
}
...
function extractParamNamesFromDoc(description) {
    if (!description)
        return [];
    const names = [];
...
function schemaToTypeExpr(schema, rootSchema) {
    ...
    if (schema.oneOf || schema.anyOf) {
        ...
        if (Array.isArray(variants)) {
            ...
            const isEnum = variants.every((v) => v.const !== undefined);
            if (isEnum) {
                return variants.map((v) => JSON.stringify(v.const)).join(' | ');
            }
    ...
    if (type === 'boolean')
        return 'boolean';
    const looksLikeObject = type === 'object' ||
...
function getMethodArgsForProperty(propertySchema, rootSchema, propertyDescription) {
    ...
    if (propertySchema &&
        ...
        const items = propertySchema.prefixItems;
        return items.map((itemSchema, index) => {
            const name = paramNames.length > 0
...
function buildTreeFromSchemas(schemas) {
    ...
    for (const moduleSchema of schemas) {
        ...
        const propsEntries = propsObj && typeof propsObj === 'object'
        ...
        const outputs = (Array.isArray(moduleSchema.outputs) ? moduleSchema.outputs : [])
            .map((o) => ({
...
function renderNodeInterfaceName(baseName) {
...
function renderParamsInterface(baseName, classSpec, indent) {
    const lines = [];
 ...
 */
function getMultiOutputInterfaceName(moduleSchema) {
    const parts = moduleSchema.name.split('.').filter((p) => p.length > 0);
 ...
 */
function getFactoryReturnType(moduleSchema) {
    const outputs = moduleSchema.outputs || [];
    if (outputs.length === 0) {
        return 'void';
    }
...
function renderFactoryFunction(moduleSchema, _interfaceName, indent) {
    ...
    lines.push(`${indent}export function ${functionName}(${args.join(', ')}): ${returnType};`);
    return lines;
}
function getQualifiedNodeInterfaceType(moduleName) {
...
function renderInterface(baseName, classSpec, indent) {
    const lines = [];
...
function renderTree(node, indentLevel = 0) {
...
//# sourceMappingURL=typescriptLibGen.js.map