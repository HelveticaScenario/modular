...
use mi_plaits_dsp::engine::{
    Engine, EngineParameters, TriggerState, virtual_analog_engine::VirtualAnalogEngine,
};
...
#[serde(default)]
struct VirtualAnalogOscillatorParams {
    /// frequency in v/oct
    freq: Signal,
    /// timbre parameter (0-1): variable square, from narrow pulse to full square to hardsync formants
    timbre: Signal,
    /// morph parameter (0-1): variable saw, from triangle to saw with an increasingly wide notch
    morph: Signal,
    /// harmonics parameter (0-1): detuning between the two waves
    harmonics: Signal,
...
#[module("mi.va", "Virtual analog oscillator with two classic waveforms")]
#[args(freq)]
pub struct VirtualAnalogOscillator {
    outputs: VirtualAnalogOscillatorOutputs,
    engine: Option<VirtualAnalogEngine>,
    ...
    params: VirtualAnalogOscillatorParams,
}

impl VirtualAnalogOscillator {
    fn update(&mut self, sample_rate: f32) {
        // Initialize engine if needed
        if self.engine.is_none() || (self.sample_rate - sample_rate).abs() > 0.1 {
            let mut engine = VirtualAnalogEngine::new(BLOCK_SIZE);
...
message_handlers!(impl VirtualAnalogOscillator {});
