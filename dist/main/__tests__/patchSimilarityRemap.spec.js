...
function graph({ modules, tracks = [], scopes = [] }) {
    return {
        modules: modules.map((m) => ({ ...m })),
        scopes,
    };
}
test('reconcilePatchBySimilarity: no currentGraph => no remaps', (t) => {
    const desired = graph({
        modules: [{ id: 'sine-1', moduleType: 'sine', params: { freq: 440 } }],
    });
    const { moduleIdRemap, appliedPatch } = reconcilePatchBySimilarity(desired, null);
    t.deepEqual(moduleIdRemap, {});
    t.is(appliedPatch, desired);
});
test('reconcilePatchBySimilarity: matches by params when ids differ', (t) => {
    const current = graph({
        modules: [{ id: 'a', moduleType: 'sine', params: { freq: 440, phase: 0 } }],
    });
    const desired = graph({
        modules: [{ id: 'b', moduleType: 'sine', params: { freq: 440, phase: 0 } }],
    });
    const { moduleIdRemap, appliedPatch } = reconcilePatchBySimilarity(desired, current, {
        matchThreshold: 0.9,
...
test('reconcilePatchBySimilarity: does not match across module types', (t) => {
    const current = graph({
...
test('reconcilePatchBySimilarity: ambiguity guard rejects ties', (t) => {
    const current = graph({
        modules: [
...
test('reconcilePatchBySimilarity: returns multiple remaps when confident', (t) => {
    const current = graph({
        modules: [
...
test('reconcilePatchBySimilarity: downstream usage disambiguates identical params', (t) => {
    ...
    const current = graph({
        modules: [
            ...
            { id: 'b', moduleType: 'sine', params: { freq: 440 } },
            {
                id: 'filter-1',
                moduleType: 'filter',
                params: { input: { type: 'cable', module: 'a', port: 'output' } },
            },
        ],
    });
    const desired = graph({
        modules: [
            ...
            { id: 'y', moduleType: 'sine', params: { freq: 440 } },
            {
                ...
                id: 'filter-1',
                moduleType: 'filter',
                params: { input: { type: 'cable', module: 'x', port: 'output' } },
            },
        ],
    });
    const { moduleIdRemap } = reconcilePatchBySimilarity(desired, current, {
...
test('reconcilePatchBySimilarity: stress test large graph', (t) => {
    t.timeout(10_000);
    const N = 150;
    const perm = (i) => (i * 73) % N;
...
//# sourceMappingURL=patchSimilarityRemap.spec.js.map