...
use mi_plaits_dsp::engine::{
    Engine, EngineParameters, TriggerState, snare_drum_engine::SnareDrumEngine,
};
...
#[serde(default)]
struct SnareDrumOscillatorParams {
    freq: Signal,
    timbre: Signal,
    morph: Signal,
    harmonics: Signal,
...
#[module("mi.snare", "Snare drum synthesis")]
#[args(freq)]
pub struct SnareDrumOscillator {
    outputs: SnareDrumOscillatorOutputs,
    engine: Option<SnareDrumEngine>,
    buffer_out: Vec<f32>,
    buffer_aux: Vec<f32>,
    buffer_pos: usize,
    last_sync: f32,
    ...
    morph: Clickless,
    harmonics: Clickless,
    params: SnareDrumOscillatorParams,
}

impl SnareDrumOscillator {
    fn update(&mut self, sample_rate: f32) {
        if self.engine.is_none() || (self.sample_rate - sample_rate).abs() > 0.1 {
            let mut engine = SnareDrumEngine::new();
...
message_handlers!(impl SnareDrumOscillator {});
