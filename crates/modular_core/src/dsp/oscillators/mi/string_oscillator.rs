...
use mi_plaits_dsp::engine::{Engine, EngineParameters, TriggerState, string_engine::StringEngine};
...
#[module("mi.string", "Inharmonic string modeling")]
#[args(freq)]
pub struct StringOscillator {
    outputs: StringOscillatorOutputs,
    engine: Option<StringEngine>,
    buffer_out: Vec<f32>,
    buffer_aux: Vec<f32>,
    buffer_pos: usize,
    last_sync: f32,
    ...
    morph: Clickless,
    harmonics: Clickless,
    params: StringOscillatorParams,
}

impl StringOscillator {
    fn update(&mut self, sample_rate: f32) {
        if self.engine.is_none() || (self.sample_rate - sample_rate).abs() > 0.1 {
            let mut engine = StringEngine::new(BLOCK_SIZE);
...
message_handlers!(impl StringOscillator {});
