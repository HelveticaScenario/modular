...
fn from_graph_seq_module_ref_pattern_is_rejected() {
    let graph = make_graph(vec![
        ("src", "$signal", json!({ "source": 2.5 })),
        ("seq", "$cycle", json!({ "pattern": "module(src:output:0)" })),
    ]);
    let err = match Patch::from_graph(&graph, SAMPLE_RATE) {
        Ok(_) => panic!("module(...) patterns should be rejected"),
        Err(err) => err,
    };
    assert!(
...
#[test]
fn from_graph_seq_sample_and_hold_module_ref_pattern_is_rejected() {
    let graph = make_graph(vec![
        ("src", "$signal", json!({ "source": 2.5 })),
        ("seq", "$cycle", json!({ "pattern": "module(src:output:0)=" })),
    ]);
