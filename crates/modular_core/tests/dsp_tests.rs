...
/// (e.g. "sine", "saw", "pulse", "lpf", "hpf", "bpf",
...
fn test_scale_and_shift_module() {
    ...
    if let Some(scaler) = patch.sampleables.get("scaler") {
        ...
        let _ = scaler.update_param(&"scale".to_string(), &InternalParam::Volts { value: 0.5 });
    }
...
fn test_pulse_oscillator_produces_output() {
    ...
    if let Some(module) = patch.sampleables.get("pulse-1") {
        ...
        let _ = module.update_param(&"width".to_string(), &InternalParam::Volts { value: 2.5 });
    }
...
fn test_noise_module_colors_produce_output() {
    ...
    assert!(
        white_max > 0.01,
        "White noise should produce non-zero output"
    );
...
fn test_lowpass_filter() {
    ...
    if let Some(lp) = patch.sampleables.get("lp") {
        ...
        let _ = lp.update_param(&"cutoff".to_string(), &InternalParam::Volts { value: 1.0 });
    }
...
fn test_highpass_filter() {
    ...
    if let Some(hp) = patch.sampleables.get("hp") {
        ...
        let _ = hp.update_param(&"cutoff".to_string(), &InternalParam::Volts { value: 8.0 });
    }
...
fn test_adsr_envelope_progression() {
    ...
    if let Some(gate) = patch.sampleables.get("gate") {
        let _ = gate.update_param(&"source".to_string(), &InternalParam::Volts { value: 5.0 });
    }
    ...
    if let Some(env) = patch.sampleables.get("env") {
        ...
        let _ = env.update_param(
            &"attack".to_string(),
            &InternalParam::Volts { value: 0.001 },
        );
        ...
        let _ = env.update_param(
            &"release".to_string(),
            &InternalParam::Volts { value: 0.005 },
        );
    }
