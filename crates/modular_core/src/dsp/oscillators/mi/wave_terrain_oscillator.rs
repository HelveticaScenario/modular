use mi_plaits_dsp::engine2::wave_terrain_engine::WaveTerrainEngine;
use napi::Result;
...
#[serde(default)]
struct WaveTerrainOscillatorParams {
    freq: Signal,
    timbre: Signal,
    morph: Signal,
    harmonics: Signal,
...
struct WaveTerrainOscillatorOutputs {
...
#[module("mi.waveterrain", "Wave terrain synthesis")]
#[args(freq)]
pub struct WaveTerrainOscillator<'a> {
    outputs: WaveTerrainOscillatorOutputs,
    engine: Option<WaveTerrainEngine<'a>>,
    buffer_out: Vec<f32>,
    buffer_aux: Vec<f32>,
    buffer_pos: usize,
    last_sync: f32,
    ...
    morph: Clickless,
    harmonics: Clickless,
    params: WaveTerrainOscillatorParams,
}

impl<'a> WaveTerrainOscillator<'a> {
    fn update(&mut self, sample_rate: f32) {
        if self.engine.is_none() || (self.sample_rate - sample_rate).abs() > 0.1 {
            let mut engine = WaveTerrainEngine::new(BLOCK_SIZE);
...
message_handlers!(impl WaveTerrainOscillator {});
