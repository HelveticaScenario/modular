 ...
 */
function extractInterpolationRegions(sourcePattern, evaluatedPattern) {
...
(0, ava_1.default)('extractInterpolationRegions: handles single interpolation at start', (t) => {
    // ${note} c e where note='g'
    const regions = extractInterpolationRegions('${note} c e', 'g c e');
    t.deepEqual(regions, [
        { sourceStart: 0, sourceEnd: 7, sourceLen: 7, evaluatedStart: 0, evaluatedLen: 1 },
    ]);
});
(0, ava_1.default)('extractInterpolationRegions: handles single interpolation in middle', (t) => {
    // a ${x} b where x='XX'
    const regions = extractInterpolationRegions('a ${x} b', 'a XX b');
    t.deepEqual(regions, [
        { sourceStart: 2, sourceEnd: 6, sourceLen: 4, evaluatedStart: 2, evaluatedLen: 2 },
    ]);
});
(0, ava_1.default)('extractInterpolationRegions: handles single interpolation at end', (t) => {
    // a b ${x} where x='c'
    const regions = extractInterpolationRegions('a b ${x}', 'a b c');
    t.deepEqual(regions, [
        { sourceStart: 4, sourceEnd: 8, sourceLen: 4, evaluatedStart: 4, evaluatedLen: 1 },
    ]);
});
(0, ava_1.default)('extractInterpolationRegions: handles multiple interpolations', (t) => {
    // a ${x} b ${y} c where x='XX', y='YYY'
    const regions = extractInterpolationRegions('a ${x} b ${y} c', 'a XX b YYY c');
    t.deepEqual(regions, [
        { sourceStart: 2, sourceEnd: 6, sourceLen: 4, evaluatedStart: 2, evaluatedLen: 2 },
        { sourceStart: 9, sourceEnd: 13, sourceLen: 4, evaluatedStart: 7, evaluatedLen: 3 },
    ]);
});
(0, ava_1.default)('extractInterpolationRegions: returns null for patterns without interpolations', (t) => {
    const regions = extractInterpolationRegions('a b c', 'a b c');
    t.is(regions, null);
});
(0, ava_1.default)('extractInterpolationRegions: handles empty interpolation result', (t) => {
    // ${x} b where x=''
    const regions = extractInterpolationRegions('${x} b', ' b');
    t.deepEqual(regions, [
        { sourceStart: 0, sourceEnd: 4, sourceLen: 4, evaluatedStart: 0, evaluatedLen: 0 },
    ]);
});
// position mapping tests
(0, ava_1.default)('buildPositionMapper: maps positions for single interpolation at start', (t) => {
    // ${note} c e where note='g'
    const regions = extractInterpolationRegions('${note} c e', 'g c e');
...
(0, ava_1.default)('buildPositionMapper: maps positions for interpolation in middle', (t) => {
    // a ${x} b where x='XX'
    const regions = extractInterpolationRegions('a ${x} b', 'a XX b');
    const map = buildPositionMapper(regions);
...
(0, ava_1.default)('buildPositionMapper: maps positions for multiple interpolations', (t) => {
    // a ${x} b ${y} c where x='XX', y='YYY'
    const regions = extractInterpolationRegions('a ${x} b ${y} c', 'a XX b YYY c');
    const map = buildPositionMapper(regions);
...
//# sourceMappingURL=interpolationMapping.spec.js.map