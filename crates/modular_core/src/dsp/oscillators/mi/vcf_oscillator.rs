...
use mi_plaits_dsp::{
    ...
    engine2::virtual_analog_vcf_engine::VirtualAnalogVcfEngine,
};
// use mi_plaits_dsp::engine2::{
// use mi_plaits_dsp::engine2::virtual_analog_vcf_engine::VirtualAnalogVcfEngine;
//     virtual_analog_vcf_engine::VirtualAnalogVcfEngine, Engine, EngineParameters, TriggerState,
// };
...
#[module("mi.vavcf", "Virtual analog with VCF")]
#[args(freq)]
pub struct VcfOscillator {
    outputs: VcfOscillatorOutputs,
    engine: Option<VirtualAnalogVcfEngine>,
    buffer_out: Vec<f32>,
    buffer_aux: Vec<f32>,
    buffer_pos: usize,
    last_sync: f32,
    ...
    morph: Clickless,
    harmonics: Clickless,
    params: VcfOscillatorParams,
}

impl VcfOscillator {
    fn update(&mut self, sample_rate: f32) {
        if self.engine.is_none() || (self.sample_rate - sample_rate).abs() > 0.1 {
            let mut engine = VirtualAnalogVcfEngine::new();
...
message_handlers!(impl VcfOscillator {});
