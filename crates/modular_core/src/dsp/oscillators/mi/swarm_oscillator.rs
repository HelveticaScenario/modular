...
use mi_plaits_dsp::engine::{Engine, EngineParameters, TriggerState, swarm_engine::SwarmEngine};

const BLOCK_SIZE: usize = 1;

#[derive(Deserialize, Default, JsonSchema, Connect)]
#[serde(default)]
struct SwarmOscillatorParams {
    freq: Signal,
    timbre: Signal,
    morph: Signal,
    harmonics: Signal,
...
#[module("mi.swarm", "Granular swarm synthesis")]
#[args(freq)]
pub struct SwarmOscillator {
    outputs: SwarmOscillatorOutputs,
    engine: Option<SwarmEngine>,
    buffer_out: Vec<f32>,
    buffer_aux: Vec<f32>,
    buffer_pos: usize,
    last_sync: f32,
    ...
    morph: Clickless,
    harmonics: Clickless,
    params: SwarmOscillatorParams,
}

impl SwarmOscillator {
    fn update(&mut self, sample_rate: f32) {
        if self.engine.is_none() || (self.sample_rate - sample_rate).abs() > 0.1 {
            let mut engine = SwarmEngine::new();
...
message_handlers!(impl SwarmOscillator {});
