...
registerIPCHandler('SYNTH_SET_FOLLOW_MODE', (enabled: boolean) => {
    synth.setFollowMode(enabled);
});
...
app.on('ready', () => {
    // Probe the local network to trigger macOS Local Network permission prompt.
    // Ableton Link uses mDNS (multicast 224.0.0.251:5353) for peer discovery,
    // but the C++ SDK's networking may not trigger the TCC prompt on its own.
    // Sending a single UDP multicast packet from the main process ensures macOS
    // shows the "allow local network access?" dialog.
    if (process.platform === 'darwin') {
        ...
        sock.bind(0, () => {
            try {
                ...
                sock.send(probe, 0, 0, 5353, '224.0.0.251', () => {
                    sock.close();
                });
            } catch {
                sock.close();
