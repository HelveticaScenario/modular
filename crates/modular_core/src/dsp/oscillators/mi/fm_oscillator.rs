...
use mi_plaits_dsp::engine::{Engine, EngineParameters, TriggerState, fm_engine::FmEngine};
...
struct FmOscillatorParams {
    /// frequency in v/oct
    freq: Signal,
    /// timbre parameter (0-1): modulation index
    timbre: Signal,
    /// morph parameter (0-1): feedback (before 12 o'clock: chaotic, after 12 o'clock: rough)
    morph: Signal,
    /// harmonics parameter (0-1): frequency ratio
    harmonics: Signal,
...
struct FmOscillatorOutputs {
    ...
    sample: f32,
    #[output("aux", "sub-oscillator output", range = (-1.0, 1.0))]
    aux: f32,
}

#[derive(Default, Module)]
#[module("mi.fm", "Two operator FM synthesis")]
#[args(freq)]
pub struct FmOscillator {
    outputs: FmOscillatorOutputs,
    engine: Option<FmEngine>,
...
message_handlers!(impl FmOscillator {});
