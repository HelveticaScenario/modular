...
use mi_plaits_dsp::engine::{Engine, EngineParameters, TriggerState, noise_engine::NoiseEngine};

const BLOCK_SIZE: usize = 1;

#[derive(Deserialize, Default, JsonSchema, Connect)]
#[serde(default)]
struct ClockedNoiseOscillatorParams {
    freq: Signal,
    timbre: Signal,
    morph: Signal,
    harmonics: Signal,
...
#[module("mi.clocknoise", "Clocked noise synthesis")]
#[args(freq, timbre?, morph?, harmonics?)]
pub struct ClockedNoiseOscillator {
    outputs: ClockedNoiseOscillatorOutputs,
    ...
    morph: Clickless,
    harmonics: Clickless,
    params: ClockedNoiseOscillatorParams,
}

impl ClockedNoiseOscillator {
    fn update(&mut self, sample_rate: f32) {
        if self.engine.is_none() || (self.sample_rate - sample_rate).abs() > 0.1 {
            let mut engine = NoiseEngine::new(BLOCK_SIZE);
...
message_handlers!(impl ClockedNoiseOscillator {});
