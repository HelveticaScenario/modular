...
exports.findScopeCallEndLines = findScopeCallEndLines;
function findScopeCallEndLines(code) {
    ...
    const indexToLine = (idx) => {
        ...
        while (low <= high) {
            ...
            const nextStart = mid + 1 < lineStarts.length ? lineStarts[mid + 1] : code.length + 1;
            if (idx >= start && idx < nextStart)
                return mid + 1; // 1-based line number
            if (idx < start) {
                high = mid - 1;
            }
            else {
    ...
    const inComment = (idx) => commentRanges.some(({ start, end }) => idx >= start && idx < end);
    const results = [];
...
//# sourceMappingURL=findScopeCallEndLines.js.map