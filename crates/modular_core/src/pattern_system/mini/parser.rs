...
mod tests {
    ...
    fn test_parse_module_ref() {
        ...
        assert!(
            err.to_string()
                .contains("module(...) syntax is no longer supported"),
    ...
    fn test_parse_module_ref_sample_and_hold() {
        let err =
            parse("module(lfo-1:output:0)=").expect_err("sample-and-hold module refs should be rejected");
        assert!(
            err.to_string()
                .contains("module(...) syntax is no longer supported"),
            "unexpected error: {err:?}"
    ...
    fn test_parse_module_ref_in_sequence() {
        let err = parse("c4 module(osc:out:0) e4")
            .expect_err("module refs in sequences should be rejected");
        assert!(
            err.to_string()
                .contains("module(...) syntax is no longer supported"),
            "unexpected error: {err:?}"
