...
pub struct Remap {
    ...
    in_min: [Clickless; PORT_MAX_CHANNELS],
    in_max: [Clickless; PORT_MAX_CHANNELS],
    out_min: [Clickless; PORT_MAX_CHANNELS],
    out_max: [Clickless; PORT_MAX_CHANNELS],
    params: RemapParams,
}
...
impl Remap {
    fn update(&mut self, _sample_rate: f32) {
        ...
        for i in 0..channels as usize {
            ...
            self.in_min[i].update(in_min_val);
            self.in_max[i].update(in_max_val);
            self.out_min[i].update(out_min_val);
            self.out_max[i].update(out_max_val);
