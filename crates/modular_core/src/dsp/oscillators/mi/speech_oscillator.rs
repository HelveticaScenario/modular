...
use mi_plaits_dsp::engine::{Engine, EngineParameters, TriggerState, speech_engine::SpeechEngine};
...
#[module("mi.speech", "Speech/vowel synthesis")]
#[args(freq)]
pub struct SpeechOscillator<'a> {
    outputs: SpeechOscillatorOutputs,
    engine: Option<SpeechEngine<'a>>,
    buffer_out: Vec<f32>,
    buffer_aux: Vec<f32>,
    buffer_pos: usize,
    last_sync: f32,
    ...
    morph: Clickless,
    harmonics: Clickless,
    params: SpeechOscillatorParams,
}

impl<'a> SpeechOscillator<'a> {
    fn update(&mut self, sample_rate: f32) {
        if self.engine.is_none() || (self.sample_rate - sample_rate).abs() > 0.1 {
            let mut engine = SpeechEngine::new(BLOCK_SIZE);
...
message_handlers!(impl SpeechOscillator {});
