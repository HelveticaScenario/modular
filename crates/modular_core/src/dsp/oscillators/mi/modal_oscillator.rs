...
use mi_plaits_dsp::engine::{Engine, EngineParameters, TriggerState, modal_engine::ModalEngine};

const BLOCK_SIZE: usize = 1;

#[derive(Deserialize, Default, JsonSchema, Connect, ChannelCount)]
#[serde(default)]
struct ModalOscillatorParams {
    /// frequency in v/oct
    freq: Signal,
    /// timbre parameter (0-1): excitation brightness and dust density
    timbre: Signal,
    /// morph parameter (0-1): decay time (energy absorption)
    morph: Signal,
    /// harmonics parameter (0-1): amount of inharmonicity, or material selection
    harmonics: Signal,
...
struct ModalOscillatorOutputs {
    ...
    sample: f32,
    #[output("aux", "raw exciter signal", range = (-1.0, 1.0))]
    aux: f32,
}
...
message_handlers!(impl ModalOscillator {});
