 ...
 *
 * Prerequisites:
 *   The Vite build must exist in .vite/ (run `yarn start` once to create it).
...
export default defineConfig({
    ...
    /**
     * Serve the Vite renderer output on port 3000.
     *
     * The compiled main process loads the renderer from
     * http://localhost:3000/index.html (via VITE_DEV_SERVER_URL or static serve).
     * In dev mode (`yarn start`), this is handled by vite dev server.
    ...
    webServer: {
        command:
            'python3 -m http.server 3000 --directory .vite/renderer/main_window --bind 127.0.0.1',
        port: 3000,
